    ...
    <Card className="group relative overflow-hidden transition-all duration-500 hover:shadow-2xl hover:shadow-blue-500/10 hover:scale-[1.05] hover:-translate-y-1 bg-gradient-to-br from-background to-muted/20 border-0 shadow-lg">
      ...
      <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
        ...
        <div className="relative">
          <Icon className={`h-4 w-4 sm:h-5 sm:w-5 ${data.color} transition-all duration-300 group-hover:scale-110 group-hover:rotate-12`} />
      ...
      <CardContent className="relative">
        <div className="text-2xl sm:text-3xl font-bold bg-gradient-to-r from-foreground to-muted-foreground bg-clip-text text-transparent group-hover:from-blue-600 group-hover:to-purple-600 transition-all duration-500">
          {data.value}
        </div>
        <CardDescription className="flex items-center text-xs sm:text-sm">
          {data.changeType === 'increase' ? (
            <ArrowUp className="h-3 w-3 sm:h-4 sm:w-4 text-green-500 mr-1 animate-bounce" />
          ) : (
            <ArrowDown className="h-3 w-3 sm:h-4 sm:w-4 text-red-500 mr-1 animate-bounce" />
          )}
          <span className={cn(
          ...
          <span className="text-muted-foreground ml-1 group-hover:text-muted-foreground/80 transition-colors duration-300 hidden sm:inline">
            from last month
          </span>
          <span className="text-muted-foreground ml-1 group-hover:text-muted-foreground/80 transition-colors duration-300 sm:hidden">
            vs last month
          </span>
        </CardDescription>
      </CardContent>
